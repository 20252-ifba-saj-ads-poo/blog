...
### Entrada de dados com java.lang.IO (java25)
...
void main() {
    Double d = Converter.read("Double: ", Double::valueOf);
    println("Numero Lido: " + d);
    
    Integer i = Converter.read("Inteiro: ", Integer::valueOf);
    println("Inteiro Lido: " + i);

    LocalDate data1 = Converter.read("Data (yyyy-MM-dd)", LocalDate::parse);
    println("Data Lida: " + data1);
    
    DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd/MM/yyyy");
    LocalDate data2 = Converter.read("Data (dd/MM/yyyy)", s -> LocalDate.parse(s, formatter));
    println("Data Lida (BR): " + data2);
}

class Converter {
    public static <T> T read(String prompt, Function<String, T> converter) {
        try {
            String input = readln(prompt);
            return converter.apply(input);
        } catch (Exception e) {
            println("Erro no formato do dado. Informar novamente.");
...
- [Mitos e verdades sobre Java](youtube.com/watch?v=I3rr6DFqqy4)
